...
const CareerServices: React.FC<CareerServicesProps> = ({ services }) => {
  ...
  return (
    <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-12">
      <div className="mb-12">
        <p className="text-gray-800 italic font-medium mb-2">
          Our Professional Services
        </p>

        <div className="flex flex-col md:flex-row md:items-center justify-between">
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-8 md:mb-0 md:w-2/3">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <button className="w-48 px-2 py-3 text-sm font-semibold bg-[#22303F] border border-transparent hover:text-black hover:bg-transparent hover:border-[#22303F] text-white transition-colors duration-200">
              Consult our Experts
            </button>
            <button className="w-48 px-2 py-3 text-sm font-semibold bg-[#67D794] border border-transparent hover:text-black hover:bg-transparent hover:border-[#67D794] text-black transition-colors duration-200">
              Get Started Now!
            </button>
          </div>
        </div>
      ...
      <div className="relative overflow-hidden">
        ...
        >
          {extendedServices.map((service, index) => (
            <div
              key={index}
              className="w-[280px] sm:min-w-[320px] bg-white p-6 flex flex-col justify-between  group transition-all duration-300 ease-in-out hover:bg-[#222F3E] mr-6 select-none flex-shrink-0"
            >
              <div>
                <h3 className="text-xl font-bold text-gray-800 mb-4 group-hover:text-white transition-colors duration-300">
                  {service.title}
                </h3>
                <div className="w-12 h-[2px] bg-[#67D794] mb-6"></div>
                <p className="text-gray-600 mb-8 group-hover:text-white transition-colors duration-300">
                  {service.description}
                </p>
              </div>
